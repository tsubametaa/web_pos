<script lang="ts">
  ...
  import { ShieldAlert, ArrowRight, Sparkles } from "lucide-svelte";
  ...
  <div class="w-full max-w-[420px] z-10 transition-all duration-300">
    ...
    >
      ...
      <div class="flex flex-col items-center gap-3 text-center">
        ...
        <div>
          <h2 class="text-lg font-bold text-h-text tracking-tight mt-1">
            {appState.needSetup ? "Setup Bisnis Baru" : "Registrasi Akun"}
          </h2>
          <p class="text-xs text-p-text mt-1 leading-relaxed font-medium">
            {appState.needSetup
              ? "Langkah awal untuk memulai konfigurasi sistem kasir POS bisnis Anda."
              : "Mulai kelola kasir dan inventori Anda dalam hitungan detik."}
          </p>
        </div>
      </div>
      ...
      <form onsubmit={handleSubmit} class="flex flex-col gap-4">
        ...
        >
          <span
            >{appState.needSetup ? "Selesaikan Setup" : "Daftar & Masuk"}</span
          >
          <ArrowRight
      ...
      >
        ...
        {:else}
          <div
            class="text-xs text-p-text font-semibold flex items-center justify-center gap-1.5"
          >
            <Sparkles class="w-3.5 h-3.5 text-accent animate-pulse" />
            <span>Langkah Awal Konfigurasi Sistem POS</span>
          </div>
        {/if}
